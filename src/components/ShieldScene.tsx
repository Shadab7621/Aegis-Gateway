...
float cnoise(vec3 P){
  vec3 Pi0=floor(P),Pi1=Pi0+vec3(1.0);Pi0=mod(Pi0,289.0);Pi1=mod(Pi1,289.0);
...
function Blob() {
  ...
  const uniforms = useMemo(() => ({ u_time:{value:0}, u_intensity:{value:0.3}, u_color:{value:new Color('#7B6EFF')} }), []);
  ...
  return (
    <mesh ref={mesh} scale={1.5} onPointerOver={()=>(hover.current=true)} onPointerOut={()=>(hover.current=false)}>
...
export default function ShieldScene() {
  return (
    <div className="absolute inset-0 z-0 opacity-70">
      <Canvas camera={{position:[0,0,8],fov:15}} gl={{antialias:true,alpha:true}} style={{background:'transparent'}}>
